...
%\pagestyle{fancy}
%\lhead{\ \ \ \ \ \ \ \  \ \ \   Compressive Sensing with Low Precision Data Representation: Radio Astronomy and Beyond}
\usepackage[top=3.2cm,bottom=3.2cm,left=2.5cm,right=2.5cm,headsep=20pt,a4paper]{geometry} 
\begin{document} 
...
\begin{equation}\label{bounds_muhat}
   1/\hat{\beta}^2_{2s} \leq \mu^{[n]} \leq 1/\hat{\alpha}^2_{2s}.
\end{equation}
\end{remark}
...
\end{split}
\end{equation}
Our quantization scheme uses a stochastic approach such that $\mathbb{P}(Q(\hat{\bf v}, b)_i = \ell_j)=\frac{\ell_{j+1}-\hat{v}_i}{\ell_{j+1}-\ell_{j}}$, and hence $\mathbb{P}(Q(\hat{\bf v}, b)_i = \ell_{j+1})=1-\frac{\ell_{j+1}-\hat{v}_i}{\ell_{j+1}-\ell_{j}}$. Substituting these into \ref{quantization_error_bound1} we have
